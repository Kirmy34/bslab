...
TEST_CASE("Own Tests - 2.1", "[Part_2]") {

    printf("Testcase 2.1: Create & rename a file\n");
    ...
    // Create file
    ret = open(FILENAME, O_EXCL | O_RDWR | O_CREAT, 0666);
    REQUIRE(ret >= 0);
    ...
    // rename
    REQUIRE(rename(FILENAME, "newName") >= 0);


    // open with old Name, must fail
    ret = open(FILENAME, O_EXCL | O_RDWR, 0666);
    REQUIRE(ret < 0);

    // open with new Name
    ret = open("newName", O_EXCL | O_RDWR, 0666);
    REQUIRE(ret >= 0);

    // remove file
    REQUIRE(unlink("newName") >= 0);

    // Open file (must fail)
    REQUIRE(open("newName", O_EXCL | O_RDWR, 0666) < 0);

}

TEST_CASE("Own Tests - 2.2", "[Part_2]") {

#define FILENAME2 "Second_File"

    printf("Testcase 2.2: Rename a file to the same name of an existing file\n");
    ...
    memset(w, 0, SMALL_SIZE);
    memset(w2, 0, SMALL_SIZE);

    gen_random(w, SMALL_SIZE);
    gen_random(w2, SMALL_SIZE);
    ...
    // Rename first file to second file
    REQUIRE(rename(FILENAME, FILENAME2) >= 0);

    // Open file again
    fd = open(FILENAME2, O_EXCL | O_RDWR, 0666);
    REQUIRE(fd >= 0);

    // Read from the file, content should match original file
    REQUIRE(read(fd, r, SMALL_SIZE) == SMALL_SIZE);
    REQUIRE(memcmp(r, w, SMALL_SIZE) == 0);
...
TEST_CASE("Own Tests - 2.3", "[Part_2]") {

    printf("Testcase 2.3: Read more than File size\n");
    ...
    // Read from the file, should only return SMALL_SIZE
    REQUIRE(read(fd, r, SMALL_SIZE + 1) == SMALL_SIZE);
...
TEST_CASE("Own Tests - 2.4", "[Part_2]") {
    printf("Testcase 2.4: Truncate a file (to bigger)\n");
...
TEST_CASE("Own Tests - 2.5", "[Part_2]") {

    printf("Testcase 2.5: Truncate a file (to 0)\n");
...
TEST_CASE("Own Tests - 2.6", "[Part_2]") {

    printf("Testcase 2.6: Create a file without a name\n");

    int ret;

    // Create file (should fail)
    ret = open("", O_EXCL | O_RDWR | O_CREAT, 0666);
    REQUIRE(ret < 0);


    // remove file to be sure
    unlink("");

    // Open file (must fail)
    REQUIRE(open("", O_EXCL | O_RDWR, 0666) < 0);

}

TEST_CASE("Own Tests - 2.7", "[Part_2]") {

    printf("Testcase 2.7: Create a file with a name too long name\n");
    int ret;
    const char *LONGNAME = "DasSindGenauVierzigZeichenLangXXXXXXXXXX";
    ...
    // Create file, should fail
    ret = open(LONGNAME, O_EXCL | O_RDWR | O_CREAT, 0666);
    REQUIRE(ret <= 0);
